...
proxyof = "https://proxyof2.com/type/torrents/"

try:
    response = requests.get(url=proxyof).content
...
response_url = requests.get(url=random_proxy).url
...
response = requests.post(url=random_proxy,data=payload).url
identify_b64 = re.findall("[a-zA-Z0-9]+", response)
identify_b64.sort(key=len, reverse=True)

decode_actual_url = b64decode(identify_b64[0].encode())
decode_actual_url = decode_actual_url.decode()
